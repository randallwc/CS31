...
namespace cs31
{
    ...
    void Board::setComputerScore( int score )
    {
        if (mIsHumanTurn == false && mGameOver == false) {
            mComputerScore = mComputerScore + score;
        }
    }
    ...
    void Board::setHumanTotal( int total )
    {
        if(mIsHumanTurn == true && mGameOver == false)
        {
            mHumanTotal = total;// do you add mhumantotal + total??????????????????
        }
    }
    ...
    void Board::setComputerTotal( int total ) 
    {
        if(mIsHumanTurn == false && mGameOver == false)
        {
            mComputerTotal = total;// do you add mcomputertotal + total??????????????????
        }
    }
